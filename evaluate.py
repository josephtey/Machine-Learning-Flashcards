...
def getIRTProb(model, df):
    prob = 1 / (1 + math.exp(-(getIRTParameters(model)[0][int(df.iloc[0]['student_id'])-1] + getIRTParameters(model)[1][int(df.iloc[0]['module_id'])-1])))
    return prob
...
#efc_model, onepl_model, twopl_model, logistic_model = trainAll(t.chinese_hist.data)
